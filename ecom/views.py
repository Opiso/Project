...
def home_view(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        #     product_count_in_cart = sum(item['quantity'] for item in cart)  # Update the count based on the cart contents
...
def view_customer_view(request):
    ...
    # Paginate the customers (10 customers per page for example)
    paginator = Paginator(customers, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request,'ecom/view_customer.html',{'customers':customers}) #{'page_obj':page_obj}
...
def delete_product_view(request, pk):
    ...
    try:
        ...
        messages.error(request, f'There was an error deleting the product: {str(e)}')
...
def mpesaapi(request):
     client = MpesaClient()
     phone_number = '0706641957'
     amount = 1
     aaccount_reference = 'eMobilis'
     transaction_description = 'Payment for Web Dev'
     callback_url = 'https://darajambili.herokuap.com/express-payment'
     response = client.stk_push(phone_number, amount, aaccount_reference, transaction_description,callback_url)
